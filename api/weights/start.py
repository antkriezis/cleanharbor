"""
CleanHarbor - Weight Estimation Job Initiation

POST /api/weights/start
- Creates a weight estimation job in Supabase
- Accepts two input modes:
  - "reuse_previous": Use CSV output from a prior extraction job
  - "upload_new": Upload a new CSV file
...
def trigger_background_process(host: str, job_id: str):
    ...
    """
    try:
        url = f"https://{host}/api/weights/process"
...
class handler(BaseHTTPRequestHandler):
    """Handles weight estimation job creation."""
        ...
        try:
            ...
            if 'application/json' in content_type:
                ...
                if input_mode == 'reuse_previous':
                    ...
                    # Verify source job exists and is complete
                    try:
                    ...
                    if not source_job.get('result') or not source_job['result'].get('rows'):
                        self._error(400, 'Source job has no result data')
                        return
                    
                    # Create weight job referencing source
                    ...
                    job_data = {
                        ...
                        'csv_data': None,  # Will be derived from source
                        'filename': source_job.get('result', {}).get('filename', 'source.csv'),
                        'result': None,
                        'preview_json': None,
                        'row_errors': None,
                        'progress': 0,
                ...
                else:  # upload_new with JSON (expects subsequent multipart upload)
                    # This path is for getting an upload URL, but we'll simplify
                    # and require multipart for upload_new mode
                ...
                # Basic CSV validation - check for header line
                try:
                    csv_preview = csv_bytes[:2000].decode('utf-8', errors='ignore')
                    if 'EWC Code' not in csv_preview and 'ewc_code' not in csv_preview.lower() and 'code' not in csv_preview.lower():
                        self._error(400, 'CSV must contain an "EWC Code" or "code" column')
                        return
                except Exception:
                    self._error(400, 'Invalid CSV file: cannot decode as UTF-8')
                    return
                
                # Create weight job with uploaded CSV
                job_id = str(uuid.uuid4())
